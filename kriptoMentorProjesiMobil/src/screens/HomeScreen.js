...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      comms.forEach(c => {
        if (cCount[c.signal_id] !== undefined) cCount[c.signal_id]++;
      });
  ...
  const onPressComments = id =>
    navigation.navigate('Comments', { signalId: id });
    ...
    if (searchText.startsWith('@')) {
      ...
      if (error || !prof) {
        Alert.alert(
          ...
          `"${name}" adlı trader bulunamadı.`
        );
      } else {
        navigation.navigate('PublicProfile', { userId: prof.user_id });
      }
    }
  };
        ...
        <View style={styles.actionsRow}>
          ...
          >
            <Ionicons
              name="thumbs-up"
              size={20}
              color={r.myType === 'like' ? '#34a853' : '#666'}
            />
          ...
          >
            <Ionicons
              name="thumbs-down"
              size={20}
              color={r.myType === 'dislike' ? '#ea4335' : '#666'}
            />
